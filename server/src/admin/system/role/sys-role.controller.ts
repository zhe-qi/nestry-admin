...
export class SysRoleController {
  ...
  @Get('/list')
  async listRole(@Query() q: QuerySysRoleDto) {
  ...
  async addAllRoleUser(@Query('roleId', ParseIntPipe) roleId: number, @Query('userIds', ParseIntArrayPipe) userIds: number[]) {
  ...
  async cancelAllRoleUser(@Query('roleId', ParseIntPipe) roleId: number, @Query('userIds', ParseIntArrayPipe) userIds: number[]) {
  ...
  @RequirePermission('system:role:edit')
  @Put('authUser/cancel')
  async cancelRoleUser(@Body('roleId', ParseIntPipe) roleId: number, @Body('userId', ParseIntPipe) userIds: number) {
    await this.roleService.cancelRoleUser(roleId, [userIds]);
    return Result.ok();
  }
  ...
  @Post('/')
  async addRole(@Body() sysRole: CreateSysRoleDto, @Req() req) {
    sysRole = {
      ...sysRole,
      dataScope: '5', // 自定义权限
  ...
  @RequirePermission('system:role:edit')
  @Put('/')
  async updateRole(@Body() sysRole: UpdateSysRoleDto, @Req() req) {
  ...
  @RequireRole('admin')
  @Put('/dataScope')
  async dataScope(@Body() sysRole: UpdateSysRoleDto, @Req() req) {
    sysRole = {
      ...sysRole,
      updateTime: nowDateTime(),
      updateBy: req.user?.userName,
  ...
  @RequirePermission('system:role:edit')
  @Put('/changeStatus')
  async updateRoleStatus(@Body() sysRole: UpdateSysRoleStatusDto) {
  ...
  @Delete('/:ids')
  async delRole(@Param('ids', ParseIntArrayPipe) roleIds: number[]) {
